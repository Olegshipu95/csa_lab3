...
Ниже описаны пример основных стековые операции в виде (stack_before -- stack_after) с некоторыми пояснениями.
...
* ```dup``` - (n1 -- n1 n1) -
* ```key``` - (n1 -- n2) - ввод 
* ```omit``` - (n1 n2 -- ) - вывести ASCII символ с кодом n1
* ```read``` - (n1 -- n2) - прочитать значение и положить на стек
